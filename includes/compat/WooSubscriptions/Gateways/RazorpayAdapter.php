<?php
/**
 * Plugin Name - WooCommerce Subscriptions Razorpay Gateway Adapter
 *
...
/**
 * Adapter bridging WooCommerce Subscriptions Razorpay hooks to WPSubscription integration.
 *
 * @since 1.0.0
 */
class RazorpayAdapter {
	...
	private function __construct() {
		...
		// Also listen to Razorpay-specific scheduled payment hook if available.
		if ( ! has_action( 'woocommerce_scheduled_subscription_payment_razorpay' ) ) {
			add_action( 'woocommerce_scheduled_subscription_payment_razorpay', array( $this, 'handle_razorpay_scheduled_payment' ), 10, 2 );
		}
	}

	/**
	 * Handle WooCommerce Subscriptions-style scheduled payment for Razorpay.
	 *
	 * @since 1.0.0
	 *
	...
	public function handle_scheduled_payment( $subscription_id, $amount = 0.0 ) {
		// Only handle if this is a Razorpay subscription.
		if ( ! $this->is_razorpay_subscription( $subscription_id ) ) {
			return;
		}
	...
	/**
	 * Handle Razorpay-specific scheduled payment hook.
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @return void
	 */
	public function handle_razorpay_scheduled_payment( $subscription_id, $amount = 0.0 ) {
		$this->process_renewal( $subscription_id );
	}
	...
	private function process_renewal( $subscription_id ) {
		...
		/**
		 * Trigger Razorpay renewal payment processing.
		 *
		 * @since 1.0.0
		 *
		 ...
		 * @param int       $subscription_id WPSubscription ID.
		 */
		do_action( 'wps_razorpay_process_renewal_payment', $renewal_order, $wps_subscription_id );

		// If Razorpay plugin is available, try to process payment directly.
		if ( class_exists( 'WC_Razorpay' ) ) {
			$this->process_razorpay_renewal( $renewal_order, $wps_subscription_id );
		}
	}

	/**
	 * Process Razorpay renewal payment.
	 *
	 * @since 1.0.0
	 *
	 * @param \WC_Order $renewal_order Renewal order.
	 ...
	 * @return void
	 */
	private function process_razorpay_renewal( $renewal_order, $subscription_id ) {
		// Check if Razorpay subscription ID exists.
		$razorpay_subscription_id = get_post_meta( $subscription_id, '_razorpay_subscription_id', true );
		...
		if ( ! $razorpay_subscription_id ) {
			...
			if ( $original_order_id ) {
				...
				if ( $original_order ) {
					$razorpay_subscription_id = $original_order->get_meta( '_razorpay_subscription_id' );
				}
			}
		}
		...
		if ( ! $razorpay_subscription_id ) {
			/**
			 * Trigger action for manual Razorpay renewal processing.
			 *
			 * @since 1.0.0
			 *
			 * @param \WC_Order $renewal_order Renewal order.
			 * @param int       $subscription_id WPSubscription ID.
			 */
			do_action( 'wps_razorpay_renewal_requires_manual_processing', $renewal_order, $subscription_id );
			return;
		}

		// Note: Razorpay API does not auto-retry, so renewal needs to be triggered manually or via webhook.
		// This adapter sets up the renewal order, but actual payment should be handled by Razorpay webhooks
		// or a separate cron job that processes pending Razorpay renewal orders.
	}

	/**
	 * Check if subscription uses Razorpay payment method.
	 *
	 * @since 1.0.0
	 *
	 * @param int $subscription_id Subscription ID.
	 ...
	 * @return bool
	 */
	private function is_razorpay_subscription( $subscription_id ) {
		...
		$razorpay_methods = array(
			'razorpay',
			'razorpay_subscriptions',
		);

		return in_array( $payment_method, $razorpay_methods, true );
	}
